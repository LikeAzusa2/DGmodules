...
@Mod.EventBusSubscriber(modid = DGModules.MODID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
public final class CataclysmShockwaveRenderer {
    private static final RenderStateShard.TransparencyStateShard LIGHTNING_TRANSPARENCY = new RenderStateShard.TransparencyStateShard(
            "dgmodules_shockwave_transparency",
            () -> {
    ...
    private static final RenderStateShard.OutputStateShard PARTICLES_TARGET = new RenderStateShard.OutputStateShard(
            "dgmodules_particles_target",
            () -> {
    ...
    private static void drawRing(Matrix4f matrix, VertexConsumer consumer, Vec3 center, float y, float radius, float thickness, int red, int green, int blue, float alpha, float angleOffset, int segments) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            consumer.vertex(matrix, ix0, y, iz0).color(red, green, blue, a).endVertex();
            consumer.vertex(matrix, ox0, y, oz0).color(red, green, blue, 0).endVertex();
            consumer.vertex(matrix, ox1, y, oz1).color(red, green, blue, 0).endVertex();
            consumer.vertex(matrix, ix1, y, iz1).color(red, green, blue, a).endVertex();
        }
    }
